...
exports.forModule = function (require, module, impl, initImpl) {
	...
	module.exports.for = function (CALLING_API) {
		function init () {
			try {
				var exports = {};
	...
	return getPackageRoot(function (err, packageRoot) {
		if (err) return callback(err);
						...
						if (m) {
							...
							return loadMetaForPackage(apiPackageRoot, function (err, apiDescriptor, apiConfig) {
								...
								if (
									...
									return callback(new Error("Package '" + apiPackageRoot + "' does not declare API '" + m[3] + "' used by '" + packageRoot + "' by mapping '" + apiConfig.api.consumes[name] + "' to '" + name + "'"));
								}
...
//						if (!/\//.test(config.api.consumes[name])) {
							API.__pgl_meta.API[name] = {
								uid: config.api.consumes[name],
								filepath: require.resolve(config.api.consumes[name])
							};
							API[name] = require(config.api.consumes[name]);
							return callback(null);
//						} else {
